...
module.exports = {
  ...
  makePlainTextNotification: function (connection, unhealthyTargets) {
    ...
    // Build fields for each unhealthy target
    var fields = [];
    unhealthyTargets.forEach((target, index) => {
      fields.push({
        "type": "mrkdwn",
        "text": `*Target ${index + 1}:*\n${target.target}\n*Health:* ${target.health}\n*Upstream ID:* ${_.get(target, 'upstream.id', 'N/A')}`
      });
    });

    // Return structured object for Web API with enhanced markdown
    return {
      text: `Upstream Health Check Failed: *${connection.name}*`,
      blocks: [
        {
          "type": "header",
          "text": {
            "type": "plain_text",
            "text": "Upstream Health Check Failed"
          }
        },
        {
          "type": "section",
          "fields": [
            ...
            {
              "type": "mrkdwn",
              "text": `*Targets com falha:*\n${unhealthyTargets.length}`
            }
          ]
        },
        {
        ...
        },
        {
          ...
          "elements": [
            {
              "type": "mrkdwn",
              "text": "Verifique a saúde dos targets upstream e suas configurações"
            }
          ]
        }
      ]
